...
def process_order(items):
    ...
    total = 0
    for item in items:
        total += compute_total(item["price"], item["quantity"], item.get("discount", 0))
...
def generate_invoice(order_id, items):
    ...
    for item in items:
        line_total = compute_total(item["price"], item["quantity"], item.get("discount", 0))
        lines.append(f"  {item['name']}: {format_currency(line_total)}")
    lines.append(f"Total: {format_currency(compute_total(sum(i['price'] * i['quantity'] for i in items), 1, 0))}")
    return "\n".join(lines)
